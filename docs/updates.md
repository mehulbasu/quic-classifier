...
## 11/10
Added train_xgboost.py implementing a Dask + multi-GPU XGBoost training pipeline that mirrors the existing cuML pattern. The script auto-detects GPU count (with override), ingests weekly Parquet data via dask_cudf, repartitions for VRAM safety, reproduces the feature engineering from load_day.py using partition-wise cuDF operations, label-encodes on GPU, trains with tree_method='hist'/device='cuda', saves the model to datasets/cache/models, and evaluates on a configurable holdout day while printing timing and metrics.